...
public class FolderAPI extends APIService {
    ...
    @Timed
    public ResponseEntity<Object> removeFolder(@PathVariable final String id, final OutputStream output) {
        try {
        ...
        return output -> {
            try (final JsonGenerator generator = mapper.getFactory().createGenerator(output)) {
                ...
                final Flux<EnrichedPreparation> preparations = Flux
                        .from(CommandHelper.toPublisher(Preparation.class, mapper, listPreparations)) // From preparation list
