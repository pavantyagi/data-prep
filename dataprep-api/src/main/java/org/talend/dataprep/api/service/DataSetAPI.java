...
public class DataSetAPI extends APIService {
    ...
    public Callable<Stream<EnrichedDataSetMetadata>> listSummary(
        ...
        return () -> {
            ...
            AtomicLong mapTime = new AtomicLong();
            return Flux.from(CommandHelper.toPublisher(DataSetMetadata.class, mapper, listDataSets)) //
                    .map(m -> {
                        LOG.info("Handle {}", m.getId());
                        ...
                        try {
                            ...
                            return Flux.from(CommandHelper.toPublisher(Preparation.class, mapper, getPreparations)).collectList() //
                                    .map(preparations -> new EnrichedDataSetMetadata(m,
                                            preparations.stream().filter(p -> p.getSteps() != null).collect(Collectors.toList()))) //
                                    .block();
                        } finally {
                            mapTime.addAndGet(System.currentTimeMillis() - start);
                        }
                    }) //
                    .toStream(1) //
                    .onClose(() -> LOG.info("Time spent: {} ms", mapTime.get()));
        };
    }
    ...
    @Timed
    public ResponseEntity<StreamingResponseBody> listCompatibleDatasets(
    ...
    public Stream<Preparation> listCompatiblePreparations(
        ...
        final Mono<List<DataSetMetadata>> compatibleList = Flux
                .from(CommandHelper.toPublisher(DataSetMetadata.class, mapper, compatibleDataSetList)) //
                .collectList() //
        ...
        return Flux.from(CommandHelper.toPublisher(Preparation.class, mapper, preparationList)) //
    ...
    @Timed
    @PublicAPI
    public ResponseEntity<StreamingResponseBody> listEncodings() {
    ...
    @Timed
    @PublicAPI
    public ResponseEntity<StreamingResponseBody> listImports() {
